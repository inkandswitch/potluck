  ...
  columns.forEach((from, fromIndex) => {
    columns.forEach((to, toIndex) => {
      ...
      relationshipTypes.forEach((relationshipType: RelationshipType) => {
        const relationships: Relationship[] = relationshipType.getRelationships(
          from,
          to,
          sortedSnippets
        );
...
export function findMatches(
  ...
  columns.forEach((column, index) => {
    if (index === 0) {
      const typeMatches: Match[] = sortedSnippets
        .filter(
          ({ type, span }) =>
...
export const AdjacentTokenRelationshipType: RelationshipType = {
  getRelationships(
    ...
    const typeBSnippetsBetweenFromAndTo = sortedSnippets.filter(
      ({ type, span }) =>
